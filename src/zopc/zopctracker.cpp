...
CzOPCTracker::CzOPCTracker(std::string strWalletFile)
{
...
CzOPCTracker::~CzOPCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzOPCTracker::Init()
{
...
bool CzOPCTracker::Archive(CMintMeta& meta)
{
...
bool CzOPCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzOPCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzOPCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzOPCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzOPCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzOPCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzOPCTracker::GetSerialHashes()
{
...
CAmount CzOPCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzOPCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzOPCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzOPCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzOPCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzOPCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzOPCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzOPCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzOPCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzOPCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzOPCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzOPCWallet* zOPCWallet)
{
    bool iszOPCWalletInitialized = (NULL != zOPCWallet);
    CMintMeta meta;
    ...
    if (! iszOPCWalletInitialized)
        zOPCWallet = new CzOPCWallet(strWalletFile);
    meta.isSeedCorrect = zOPCWallet->CheckSeed(dMint);
    if (! iszOPCWalletInitialized)
        delete zOPCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzOPCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzOPCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzOPCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzOPCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzOPCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzOPCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzOPCWallet* zOPCWallet = new CzOPCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zOPCWallet);
        }
        delete zOPCWallet;
        LogPrint("zero", "%s: added %d dzopc from DB\n", __func__, listDeterministicDB.size());
    }
