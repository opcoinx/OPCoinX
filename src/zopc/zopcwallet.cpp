...
CzOPCWallet::CzOPCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zopc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZOPCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZOPCSeed_deprecated()) {
                    LogPrintf("%s: Updated zOPC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zopc seed\n", __func__);
                }
            }
        }
    }
...
bool CzOPCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZOPCCount(nCountLastUsed);
    else if (!walletdb.ReadZOPCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzOPCWallet::Lock()
{
    seedMaster = 0;
}

void CzOPCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzOPCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZOPC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzOPCWallet::LoadMintPoolFromDB()
{
...
void CzOPCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzOPCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzOPCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zopcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzOPCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZOPC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zopcTracker which also adds to database
    pwalletMain->zopcTracker->Add(dMint, true);
...
void CzOPCWallet::SeedToZOPC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzOPCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzOPCWallet::GenerateDeterministicZOPC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzOPCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzOPCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzOPCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
