    ...
    if(Params().NetworkID() != CBaseChainParams::REGTEST) {
        ...
        if ((nTimeBlockFrom + nMinStakeAge > nTimeTx)) // Min age requirement
            return error("CheckStakeKernelHash() : min age violation - nTimeBlockFrom=%d nStakeMinAge=%d nTimeTx=%d",
                         nTimeBlockFrom, nMinStakeAge, nTimeTx);
...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zOPCX stake block is less than allowed start height", __func__);

    if (CZOpcxStake* zOPCX = dynamic_cast<CZOpcxStake*>(stake)) {
        CBlockIndex* pindexFrom = zOPCX->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zOPCX stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zOPCX stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zOPCX->GetValue()));
        if (nChecksum200 != zOPCX->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zOPCX->GetChecksum(), nChecksum200);
    } else {
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
    ...
    if (nPreviousBlockHeight >= Params().Zerocoin_Block_V2_Start() && txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZOpcxStake(spend));

        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s: staked zOPCX fails context checks", __func__);
    } else {
        ...
        COpcxStake* opcxInput = new COpcxStake();
        opcxInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(opcxInput);
    }
    ...
    if (!txin.IsZerocoinSpend() && nPreviousBlockHeight >= SOFT_FORK_VERSION_200 - 1
        && Params().NetworkID() != CBaseChainParams::REGTEST) { //Equivalent for zOPCX is checked above in ContextualCheckZerocoinStake()
        ...
        if (nBlockFromTime + nMinStakeAge > nTxTime) // Min age requirement
            return error("CheckStakeKernelHash() : min age violation - nBlockFromTime=%d nStakeMinAge=%d nTimeTx=%d", nBlockFromTime, nMinStakeAge, nTxTime);
    }
