...
bool CheckStakeKernelHash(const CBlockIndex* pindexPrev, const unsigned int nBits, CStakeInput* stake, const unsigned int nTimeTx, uint256& hashProofOfStake, const bool fVerify)
{
    ...
    // Weighted target
    uint256 bnWeight = uint256(nValueIn) / 10000;
    bnTarget *= bnWeight;
...
bool GetHashProofOfStake(const CBlockIndex* pindexPrev, CStakeInput* stake, const unsigned int nTimeTx, const bool fVerify, uint256& hashProofOfStakeRet) {
    ...
    if (fVerify) {
        LogPrint("staking", "%s :{ nStakeModifier=%s\n"
                            ...
                            "}\n",
            __func__, HexStr(modifier_ss), ((stake->IsZOPC()) ? "Not available" : std::to_string(stake->getStakeModifierHeight())));
    }
    return true;
}
...
bool ContextualCheckZerocoinStake(int nPreviousBlockHeight, CStakeInput* stake)
{
    if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s : zOPC stake block is less than allowed start height", __func__);

    if (CZPivStake* zOPC = dynamic_cast<CZPivStake*>(stake)) {
        CBlockIndex* pindexFrom = zOPC->GetIndexFrom();
        if (!pindexFrom)
            return error("%s : failed to get index associated with zOPC stake checksum", __func__);

        int depth = (nPreviousBlockHeight + 1) - pindexFrom->nHeight;
        if (depth < Params().Zerocoin_RequiredStakeDepth())
            return error("%s : zOPC stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, nPreviousBlockHeight, pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zOPC->GetValue()));
        if (nChecksum200 != zOPC->GetChecksum())
            return error("%s : accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zOPC->GetChecksum(), nChecksum200);
    } else {
...
bool initStakeInput(const CBlock block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s : staked zOPC fails context checks", __func__);
    } else {
        ...
        CPivStake* opcxInput = new CPivStake();
