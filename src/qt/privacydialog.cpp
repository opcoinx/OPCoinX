                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zOPCX ought to be enough for anybody." - Bill Gates, 2017
    ui->zOPCXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzOPCXSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zOPCXpayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzOPCX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOPCX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zOPCX, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zOPCX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zOPCX in ") +
...
void PrivacyDialog::on_pushButtonSpendzOPCX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zOPCX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zOPCX, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zOPCX
        sendzOPCX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zOPCX
    sendzOPCX();
}

void PrivacyDialog::on_pushButtonZOpcxControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZOpcxControlDialog* zOpcxControl = new ZOpcxControlDialog(this);
    zOpcxControl->setModel(walletModel);
    zOpcxControl->exec();
}

void PrivacyDialog::setZOpcxControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzOpcxSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzOPCX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid OPCoinX Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zOPCXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zOPCX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " OPCX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zOPCX</b>";
    ...
    // use mints from zOPCX selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZOpcxControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZOpcxControlDialog::GetSelectedMints();
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zOPCX transaction
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zOPCX was spent successfully update the addressbook with the label
    ...
    // Clear zopcx selector in case it was used
    ZOpcxControlDialog::setSelectedMints.clear();
    ui->labelzOpcxSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zOPCX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Opcx, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.IsZerocoinMint())
            strStats += tr("zOPCX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zopcxTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zOPCX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zOPCX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zOPCX "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zOPCX "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zOPCX </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zOPCX </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zOPCX </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzOPCXSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzOPCX->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zOPCX
        //ui->pushButtonMintzOPCX->setEnabled(false);
        //ui->pushButtonMintzOPCX->setToolTip(tr("zOPCX is currently disabled due to maintenance."));

        // Spend zOPCX
        ui->pushButtonSpendzOPCX->setEnabled(false);
        ui->pushButtonSpendzOPCX->setToolTip(tr("zOPCX is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zOPCX
        //ui->pushButtonMintzOPCX->setEnabled(true);
        //ui->pushButtonMintzOPCX->setToolTip(tr("PrivacyDialog", "Enter an amount of OPCX to convert to zOPCX", 0));

        // Spend zOPCX
        ui->pushButtonSpendzOPCX->setEnabled(true);
        ui->pushButtonSpendzOPCX->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
