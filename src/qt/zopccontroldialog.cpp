...
#include "ui_zopccontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zopcTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zopcTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zOPC.";
            else if (!mint.isSeedCorrect)
                strReason = "The zOPC seed used to mint this zOPC is not the same as currently hold in the wallet";
            else
