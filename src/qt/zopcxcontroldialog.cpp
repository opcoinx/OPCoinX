...
#include "ui_zopcxcontroldialog.h"
...
std::set<std::string> ZOpcxControlDialog::setSelectedMints;
std::set<CMintMeta> ZOpcxControlDialog::setMints;

bool CZOpcxControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZOpcxControlDialog::COLUMN_DENOMINATION || column == ZOpcxControlDialog::COLUMN_VERSION || column == ZOpcxControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZOpcxControlDialog::ZOpcxControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZOpcxControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZOpcxControlDialog::~ZOpcxControlDialog()
{
    delete ui;
}

void ZOpcxControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOpcxControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZOpcxControlWidgetItem* itemDenom(new CZOpcxControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZOpcxControlWidgetItem *itemMint = new CZOpcxControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zopcxTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zopcxTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zOPCX.";
            else if (!mint.isSeedCorrect)
                strReason = "The zOPCX seed used to mint this zOPCX is not the same as currently hold in the wallet";
            else
...
void ZOpcxControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOpcxControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZOpcx_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZOpcxControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZOpcxControlDialog::GetSelectedMints()
{
...
void ZOpcxControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
