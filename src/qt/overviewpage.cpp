...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::OPCX)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount opcxAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // OPCX Watch-Only Balance
    ...
    // Combined balances
    CAmount availableTotalBalance = opcxAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // OPCX labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, opcxAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    if (fEnableZeromint) {
        ...
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in opcx.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in opcx.conf");
    }
    ...
    // OPCX Available
    bool showOPCXAvailable = settingShowAllBalances || opcxAvailableBalance != nTotalBalance;
    bool showWatchOnlyOPCXAvailable = showOPCXAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showOPCXAvailable || showWatchOnlyOPCXAvailable);
    ui->labelBalance->setVisible(showOPCXAvailable || showWatchOnlyOPCXAvailable);
    ui->labelWatchAvailable->setVisible(showWatchOnlyOPCXAvailable && showWatchOnly);

    // OPCX Pending
    bool showOPCXPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyOPCXPending = showOPCXPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showOPCXPending || showWatchOnlyOPCXPending);
    ui->labelUnconfirmed->setVisible(showOPCXPending || showWatchOnlyOPCXPending);
    ui->labelWatchPending->setVisible(showWatchOnlyOPCXPending && showWatchOnly);

    // OPCX Immature
    bool showOPCXImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = showOPCXImmature || watchImmatureBalance != 0;
    ui->labelImmatureText->setVisible(showOPCXImmature || showWatchOnlyImmature);
    ui->labelImmature->setVisible(showOPCXImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

    // OPCX Locked
    bool showOPCXLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyOPCXLocked = showOPCXLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showOPCXLocked || showWatchOnlyOPCXLocked);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("OPCX")
    updateDisplayUnit();
