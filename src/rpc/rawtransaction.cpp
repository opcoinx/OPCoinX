...
UniValue createrawzerocoinstake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nCreates raw zOPC coinstakes (without MN output).\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    if (!pwalletMain->CreateZOPCOutPut(staked_denom, vOutMint[1], dMint))
        throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zopc output");

    for (int i=2; i<5; i++) {
        if (!pwalletMain->CreateZOPCOutPut(libzerocoin::ZQ_ONE, vOutMint[i], dMint))
            throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zopc output");
    }
