...
#include "zopc/accumulatorcheckpoints.h"
#include "zopcchain.h"
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZopcThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the OPCX and zOPC money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-zopcstake=<n>", strprintf(_("Enable or disable staking functionality for zOPC inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzopc=<n>", strprintf(_("Enable automatic wallet backups triggered after each zOPC minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zOPC spends and stakes (0-1, default %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-zopcbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zOPC backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
        ...
        do {
            try {
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainActive.Height() > Params().Zerocoin_StartHeight() && chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    CAmount zopcSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();

                    if (pblockindex->GetZerocoinSupply() < zopcSupplyCheckpoint) {
                        // Trigger reindex due wrapping serials
                        LogPrintf("Current GetZerocoinSupply: %d vs %d\n", pblockindex->GetZerocoinSupply()/COIN , zopcSupplyCheckpoint/COIN);
                        reindexDueWrappedSerials = true;
                    } else if (pblockindex->GetZerocoinSupply() > zopcSupplyCheckpoint) {
                        // Trigger global zOPC reindex
                ...
                if (GetBoolArg("-reindexmoneysupply", false) || reindexZerocoin) {
                    if (chainHeight > Params().Zerocoin_StartHeight()) {
                        RecalculateZOPCMinted();
                        RecalculateZOPCSpent();
                    }
                ...
                if(Params().NetworkID() == CBaseChainParams::MAIN && chainActive.Height() > Params().Zerocoin_StartHeight() &&  chainHeight > Params().Zerocoin_Block_EndFakeSerial()) {
                    ...
                    CAmount zopcSupplyCheckpoint = Params().GetSupplyBeforeFakeSerial() + GetWrapppedSerialInflationAmount();
                    if (pblockindex->GetZerocoinSupply() != zopcSupplyCheckpoint)
                        return InitError(strprintf("ZerocoinSupply Recalculation failed: %d vs %d", pblockindex->GetZerocoinSupply()/COIN , zopcSupplyCheckpoint/COIN));
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzOPCWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zOPCWallet
        uiInterface.InitMessage(_("Syncing zOPC wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZPivBackups = GetBoolArg("-backupzopc", true);
        pwalletMain->setZPivAutoBackups(fEnableZPivBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zopcTracker->Init();
    ...
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZopcThread(threadGroup);
    }
    ...
    if (pwalletMain) {
        ...
        if (GetBoolArg("-precompute", false)) {
            // Run a thread to precompute any zOPC spends
