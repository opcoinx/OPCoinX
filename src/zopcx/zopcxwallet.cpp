...
CzOPCXWallet::CzOPCXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zopcx seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZOPCXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZOPCXSeed_deprecated()) {
                    LogPrintf("%s: Updated zOPCX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zopcx seed\n", __func__);
                }
            }
        }
    }
...
bool CzOPCXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZOPCXCount(nCountLastUsed);
    else if (!walletdb.ReadZOPCXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzOPCXWallet::Lock()
{
    seedMaster = 0;
}

void CzOPCXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzOPCXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZOPCX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzOPCXWallet::LoadMintPoolFromDB()
{
...
void CzOPCXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzOPCXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzOPCXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zopcxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzOPCXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZOPCX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zopcxTracker which also adds to database
    pwalletMain->zopcxTracker->Add(dMint, true);
...
void CzOPCXWallet::SeedToZOPCX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzOPCXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzOPCXWallet::GenerateDeterministicZOPCX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzOPCXWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzOPCXWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzOPCXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
