// Copyright (c) 2018-2019 The OPCX developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zopcx/deterministicmint.h>
#include "zopcxtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzOPCXTracker::CzOPCXTracker(std::string strWalletFile)
{
...
CzOPCXTracker::~CzOPCXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzOPCXTracker::Init()
{
...
bool CzOPCXTracker::Archive(CMintMeta& meta)
{
...
bool CzOPCXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzOPCXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzOPCXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzOPCXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzOPCXTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzOPCXTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzOPCXTracker::GetSerialHashes()
{
...
CAmount CzOPCXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzOPCXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzOPCXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzOPCXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzOPCXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzOPCXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzOPCXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzOPCXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzOPCXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzOPCXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzOPCXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzOPCXWallet* zOPCXWallet)
{
    bool iszOPCXWalletInitialized = (NULL != zOPCXWallet);
    CMintMeta meta;
    ...
    if (! iszOPCXWalletInitialized)
        zOPCXWallet = new CzOPCXWallet(strWalletFile);
    meta.isSeedCorrect = zOPCXWallet->CheckSeed(dMint);
    if (! iszOPCXWalletInitialized)
        delete zOPCXWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzOPCXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzOPCXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzOPCXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzOPCXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzOPCXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzOPCXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzOPCXWallet* zOPCXWallet = new CzOPCXWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zOPCXWallet);
        }
        delete zOPCXWallet;
        LogPrint("zero", "%s: added %d dzopcx from DB\n", __func__, listDeterministicDB.size());
    }
