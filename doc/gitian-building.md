...
*Setup instructions for a gitian build of OPCX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the OPCX
...
Clone the git repositories for opcx and gitian and then checkout the opcx version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the opcx repository
...
To build OPCX (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the opcx repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/opcx/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/opcx-crypto/opcx
...
./bin/gbuild --commit opcx=${COMMIT} --url opcx=${URL} ../opcx/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/opcx-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/opcx-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/opcx-build.assert
```
